...
Gemini увидит и техническое описание API, и Kotlin-каркас выше.

> Собери Android-приложение на Kotlin (Jetpack Compose, Material 3) для
> сотрудников сервиса по ремонту техники «АВИОР». Это внутренний рабочий
> инструмент — приём заказов, работа с клиентами, отслеживание статусов
> ремонта, — не публичное приложение, упор на скорость работы одной рукой
> и приятный, современный вид, не унылую форму.
>
> **Дизайн**
> Фирменные цвета: тёмно-синий #1C2436 (основной, шапки/акценты) и золотой
> #E0B45C (акцент/кнопки/подсветка активных элементов) — та же палитра,
> что в веб-версии CRM. Material 3 с этой кастомной цветовой схемой (Light
> + Dark тема, переключение по системной настройке). Скруглённые карточки,
> мягкие тени, никаких резких переходов — везде плавная анимация:
> Crossfade между экранами, AnimatedVisibility при появлении/скрытии
> элементов, анимированный переход между статусами заказа (например,
> прогресс-полоска или последовательная подсветка стадий принят →
> диагностика → согласование → в ремонте → готов → выдан), скелетоны
> (shimmer placeholder) на время загрузки вместо голого спиннера,
> pull-to-refresh с кастомной анимацией на списках.
>
> **Иконки** — по типу устройства используй те же смысловые иконки, что
> в веб-версии: 🖥️ настольный ПК, 🖥 моноблок, 💻 ноутбук, 📱 смартфон,
> 🔲 планшет, 🎮 игровая консоль, 🔌 аксессуары (Material Icons с похожим
> смыслом, если эмодзи не вписываются в стиль — иконки в духе
> `Icons.Rounded`, не Filled, для мягкости). Статусы заказа — тоже с
> цветовым кодом (согласование/диагностика — жёлтый, в ремонте — синий,
> готов — зелёный, выдан — серый, отказ — красный), как в веб-CRM.
>
> **Экраны**
> 1. **Вход** — логин/пароль → `POST /auth.php`, токен сохранить в
>    `EncryptedSharedPreferences`. Приятный экран с логотипом АВИОР,
>    плавное появление полей.
> 2. **Главная/панель** — краткая сводка (можно посчитать на клиенте из
>    `GET /orders.php`: сколько заказов в каждом статусе — как pipeline-
>    блок на панели веб-CRM) + быстрые кнопки «Новый заказ» / «Клиенты».
> 3. **Список заказов** — фильтр по статусу чипами сверху (горизонтальный
>    скролл), карточка заказа: номер, клиент, устройство с иконкой,
>    цветной бейдж статуса, сумма. `GET /orders.php?status=...`.
> 4. **Карточка заказа** — полная информация (`GET /orders.php?id=`),
>    список позиций (`order.parts`), история статусов таймлайном
>    (`order.status_log`), кнопка смены статуса (выбор из `GET /meta.php`
>    → `POST /update_status.php`) с анимированным подтверждением, кнопки
>    «Квитанция»/«Акт» — открывают `receipt_url`/`report_url` во
>    встроенном браузере (Custom Tabs) или через системное «Поделиться»
>    (`Intent.ACTION_SEND`) — это готовые публичные ссылки, показывать
>    только если поле не `null`.
> 5. **Клиенты** — поиск (`GET /clients.php?q=`) с debounce, карточка
>    клиента, создание нового (`POST /clients.php`, источник — выбор из
>    `meta.client_sources`).
> 6. **Новый заказ** — выбор существующего клиента или создание нового
>    прямо в форме, тип/модель устройства, описание проблемы, оценка
>    стоимости (`POST /orders.php`).
>
> **Технически**: Retrofit + OkHttp + Gson, модели данных и интерфейс API
> — бери из раздела «Готовый Kotlin-каркас» в этом файле, не выдумывай
> заново. Базовый URL: `https://cms.avior.moscow/api/mobile/`. Навигация
> — Jetpack Navigation Compose, нижняя панель навигации (Заказы/Клиенты/
> Новый заказ) с анимированной сменой вкладок. Состояния загрузки/ошибки/
> пустого списка — отдельные, дружелюбные экраны с иллюстрацией/иконкой,
> не просто пустой белый экран.
